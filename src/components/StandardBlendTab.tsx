...
const StandardBlendTab = ({ settings, topOffOptions }: Props): JSX.Element => {
  ...
  // Accessibility IDs
  const topGasSelectId = useId();
  const sensitivitySliderId = useId();
  ...
  return (
    <>
      ...
      <AccordionItem title="Top-Off Gas" defaultOpen={true}>
        <div className="field">
          <label htmlFor={topGasSelectId}>Select Gas</label>
          <select
            id={topGasSelectId}
      ...
      {result?.success && sensitivityAnalysis && (
        <AccordionItem title="Fill Sensitivity" defaultOpen={false}>
          <div className="field">
            <label htmlFor={sensitivitySliderId}>Adjust Start Pressure ({settings.pressureUnit.toUpperCase()})</label>
            <input
